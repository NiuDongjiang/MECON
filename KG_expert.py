...
from KGNN_R import NHGNN, HHGNN
...
class LSEblock(nn.Module):
    def __init__(self, hidden_dim=128, num_layers=3, num_heads=8, num_node_type=13):
        ...
        self.NHGNNs = nn.ModuleList([
            NHGNN(hidden_dim=hidden_dim, num_heads=num_heads, num_node_type=num_node_type)
            for _ in range(num_layers)
        ])
        self.transform = LinearBlock(hidden_dim)

    def forward(self, x, edge_index, edge_attr, node_type_index, batch):
        """
        x:              [N, D]
        edge_index:     [2, E]
        edge_attr:      [E, D]
        node_type_index:[N]
        batch:          [N]  ← 新增，PyG batch vector
        """
        for i in range(self.num_layers):
            x, edge_attr = self.NHGNNs[i](
                x,
                edge_index,
                edge_attr,
                node_type_index,
...
class HSEblock(nn.Module):
    def __init__(self, hidden_dim=128, num_layers=3, hyperNum=8, num_node_type=13):
        super().__init__()
        self.num_layers = num_layers
        self.HHGNNs = nn.ModuleList([HHGNN(hidden_dim=hidden_dim, num_node_type=num_node_type) for _ in range(num_layers)])
...
class BiSemEncoder(nn.Module):
    ...
    def forward(self, SUBKG_data1, SUBKG_data2, drug_index1, drug_index2, Trans_emb):
        KG_node_index1, KG_edge_index1, KG_edge_type_index1, KG_node_type_index1, KG_node_mask1, KG_batch1 = \
            SUBKG_data1.node_type_, SUBKG_data1.edge_index, SUBKG_data1.edge_type, SUBKG_data1.atom_type, SUBKG_data1.node_mask, SUBKG_data1.batch
        KG_node_index2, KG_edge_index2, KG_edge_type_index2, KG_node_type_index2, KG_node_mask2, KG_batch2 = \
            SUBKG_data2.node_type_, SUBKG_data2.edge_index, SUBKG_data2.edge_type, SUBKG_data2.atom_type, SUBKG_data2.node_mask, SUBKG_data2.batch
        ...
        return KG_d1_feat, KG_d2_feat, cl_loss, KG_x1_HG, KG_x1_HP, KG_x2_HG, KG_x2_HP


class KG_expert(nn.Module):
    def __init__(self, hidden_dim = 128, ddi_type = 86, num_layers = 3, hyperNum = 8, drop = 0.2):
        super(KG_expert, self).__init__()
    ...
    def forward(self, drug_index1, drug_index2, DDI_type_index, Trans_emb_HG, Trans_emb_HY, Page_SUBKG_data1, Page_SUBKG_data2, RW_SUBKG_data1, RW_SUBKG_data2):
        ...
        page_KG_d1_feat, page_KG_d2_feat, inter_cl_loss1, page_h_N, page_h_H, page_t_N, page_t_H = self.hyperDDI_page(Page_SUBKG_data1, Page_SUBKG_data2, drug_index1, drug_index2, Trans_emb_HG)
        RW_KG_d1_feat, RW_KG_d2_feat, inter_cl_loss2, RW_h_N, RW_h_H, RW_t_N, RW_t_H = self.hyperDDI_RW(RW_SUBKG_data1, RW_SUBKG_data2, drug_index1, drug_index2, Trans_emb_HY)
        ...
        return output, inter_cl_loss1, inter_cl_loss2, intra_cl_loss, KG_d1_feat, KG_d2_feat, page_h_N, page_h_H, page_t_N, page_t_H, RW_h_N, RW_h_H, RW_t_N, RW_t_H
