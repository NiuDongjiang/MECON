...
cid_db_df = pd.read_csv(
    "/tmp/MECON-main/MECON/twosides_cid_to_drugbank_restricted_updated.csv"
)
with open(f'dataset/{dataset_n}/index_map.pkl', 'rb') as f:
    INDEX_MAP = pkl.load(f)

with open(f'dataset/{dataset_n}/page_subgraph/subgraph_to_Tensor_32.pkl', 'rb') as f:
    PAGE_SUBGRAPH = pkl.load(f)

with open(f'dataset/{dataset_n}/RW_subgraph/subgraph_to_Tensor_32.pkl', 'rb') as f:
    RW_SUBGRAPH = pkl.load(f)
...
class DrugDataset(Dataset):
    def __init__(self, tri_list, ratio=1.0, neg_ent=1,seed=42 ,disjoint_split=True, shuffle=True):
        ...
        self.cid_to_drugbank = dict(
            zip(cid_db_df["twosides_cid"], cid_db_df["drugbank_id"])
        )
    ...
    def _build_kg_data(self, kg_tuple):
        node_idx, node_type, edge_idx, edge_type, node_mask = kg_tuple
        return DATA.Data(
            x=torch.zeros((node_idx.shape[0], 1)),
            node_type_=node_idx,
            edge_index=edge_idx,
            edge_type=edge_type,
            atom_type=node_type,
    ...
    def collate_fn(self, batch):
        ...
        pos_page_h, pos_page_t = [], []
        pos_rw_h, pos_rw_t = [], []

        neg_page_h, neg_page_t = [], []
        neg_rw_h, neg_rw_t = [], []
        ...
        for h, t, r in batch:
            ...
            pos_page_h.append(self._build_kg_data(self.page_subgraph[self.cid_to_drugbank[h] if isinstance(h, str) and h.startswith("CID") else h]))
            pos_page_t.append(self._build_kg_data(self.page_subgraph[self.cid_to_drugbank[t] if isinstance(t, str) and t.startswith("CID") else t]))
            pos_rw_h.append(self._build_kg_data(self.rw_subgraph[self.cid_to_drugbank[h] if isinstance(h, str) and h.startswith("CID") else h]))
            pos_rw_t.append(self._build_kg_data(self.rw_subgraph[self.cid_to_drugbank[t] if isinstance(t, str) and t.startswith("CID") else t]))
            ...
            for neg_h in neg_heads:
                ...
                neg_page_h.append(
                    self._build_kg_data(
                        self.page_subgraph[
                            self.cid_to_drugbank[neg_h] if isinstance(neg_h, str) and neg_h.startswith("CID") else neg_h
                        ]
                    )
                )
                ...
                neg_page_t.append(
                    self._build_kg_data(
                        self.page_subgraph[
                            self.cid_to_drugbank[t] if isinstance(t, str) and t.startswith("CID") else t
                        ]
                    )
                )
            ...
            for neg_t in neg_tails:
                ...
                neg_page_h.append(
                    self._build_kg_data(
                        self.page_subgraph[
                            self.cid_to_drugbank[h] if isinstance(h, str) and h.startswith("CID") else h
                        ]
                    )
                )
                ...
                neg_page_t.append(
                    self._build_kg_data(
                        self.page_subgraph[
                            self.cid_to_drugbank[neg_t] if isinstance(neg_t, str) and neg_t.startswith("CID") else neg_t
                        ]
                    )
                )
        ...
        pos_page_h = Batch.from_data_list(pos_page_h)
        pos_page_t = Batch.from_data_list(pos_page_t)
        pos_rw_h = Batch.from_data_list(pos_rw_h)
        pos_rw_t = Batch.from_data_list(pos_rw_t)

        pos_tri = (
        pos_h_samples, pos_h_fp, pos_h_desc, pos_t_samples, pos_t_fp, pos_t_desc, pos_rels, pos_h_edge_samples,
        pos_t_edge_samples, pos_page_h, pos_page_t, pos_rw_h, pos_rw_t, pos_b_samples)
        ...
        neg_page_h = Batch.from_data_list(neg_page_h)
        neg_page_t = Batch.from_data_list(neg_page_t)
        neg_rw_h = Batch.from_data_list(neg_rw_h)
        neg_rw_t = Batch.from_data_list(neg_rw_t)

        neg_tri = (
        neg_h_samples, neg_h_fp, neg_h_desc, neg_t_samples, neg_t_fp, neg_t_desc, neg_rels, neg_h_edge_samples,
        neg_t_edge_samples, neg_page_h, neg_page_t, neg_rw_h, neg_rw_t, neg_b_samples)
    ...
    def __create_graph_data(self, id):
        ...
        data = Data(x=n_features, edge_index=edge_index, edge_attr=edge_attr, DB_ID=id)
        if isinstance(id, str) and id.startswith("CID"):
            drugbank_id = self.cid_to_drugbank.get(id, None)
            if drugbank_id is None:
                raise KeyError(f"CID {id} not found in CID→DrugBank mapping")

        elif isinstance(id, str) and id.startswith("DB"):
            drugbank_id = id

        else:
            raise ValueError(f"Unrecognized drug id format: {id}")

            # ---------- DrugBank ID -> internal index ----------
        if drugbank_id not in self.index_map:
            raise KeyError(f"DrugBank ID {drugbank_id} not in index_map")

        data.drug_index = torch.tensor(
            self.index_map[drugbank_id], dtype=torch.long
        )
        return data
