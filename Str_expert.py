...
class MVN_DDI(nn.Module):
    ...
    def forward(self, h_data, h_data_fin, h_data_desc, t_data, t_data_fin, t_data_desc, rels, h_data_edge, t_data_edge, b_graph):
        ...
        for i, block in enumerate(self.blocks):
            out = block(h_data, t_data, h_data_edge, t_data_edge, b_graph)
        ...
        return scores, repr_h.mean(dim=1), repr_t.mean(dim=1), h_data, t_data
...
class MVN_DDI_Block(nn.Module):
    ...
    def forward(self, h_data, t_data, h_data_edge, t_data_edge, b_graph):
        h_x_in = h_data.x
        t_x_in = t_data.x
